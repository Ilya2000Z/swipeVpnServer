...
func addUser(deviceID string, name string, onboardInfo string) (int, error) {
	db, err := db_connect()
	if err != nil {
		return 0, err
	}
	...
	query := `INSERT INTO users (deviceid, name, onbordInfo, onborded) VALUES (?, ?, ?, 1)`
	result, err := db.Exec(query, deviceID, name, onboardInfo)
	if err != nil {
